...
class GithubStatsAkkaHttpAdapter(port: Int)(implicit val useCase: PRSummaryUseCase) extends StrictLogging {
  ...
  def serverEndpoints: List[ServerEndpoint[Any, Future]] = {
    ...
    List(
      GithubStatsWebAdapter.userSummary.serverLogic(generateUserSummary)
    )
  }
